...
void gmg_init(int nx, int ny, int nz, float *x1, float *x2, float *x3);
...
int main(int argc, char *argv[])
{
  ...
  for(i=0; i<=nx; i++) xx[i] = i*dx;
  for(j=0; j<=ny; j++) yy[j] = j*dy;
  for(k=0; k<=nz; k++) zz[k] = k*dz;
  ...
  gmg_init(nx, ny, nz, xx, yy, zz);//preconditioner initialization
